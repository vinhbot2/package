...
const { GatewayCloseCodes, GatewayDispatchEvents, Routes } = require('../../../Publish02/api');
...
class WebSocketManager extends EventEmitter {
  ...
  async connect() {
    ...
    const { url: gatewayURL, shards: recommendedShards, session_start_limit: sessionStartLimit } = await this.client.rest.get(Routes.gatewayBot()).catch(error => {
      throw error.status === 401 ? invalidToken : error;
    });
    const { total, remaining } = sessionStartLimit;
    this.debug(`Thông tin về cổng đã tìm nạp: URL: ${gatewayURL} Các phân đoạn được đề xuất: ${recommendedShards}`);
    this.debug(`Thông tin giới hạn phiên: Tổng cộng: ${total}, Còn lại: ${remaining}`);
    this.gateway = `${gatewayURL}/`;
    let { shards } = this.client.options;

    if (shards === 'auto') {
      this.debug(`Sử dụng số lượng phân đoạn được đề xuất do Discord cung cấp: ${recommendedShards}`);
      this.totalShards = this.client.options.shardCount = recommendedShards;
      shards = this.client.options.shards = Array.from({ length: recommendedShards }, (_, i) => i);
    }

    this.totalShards = shards.length;
    this.debug(`Các phân đoạn sinh sản: ${shards.join(', ')}`);
